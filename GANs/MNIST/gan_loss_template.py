...
def gan_loss_discriminator(Discriminator, Generator, x_real, z):
    ...
    D_real_loss =  torch.nn.BCELoss()(Discriminator(x_real) , torch.ones((batch_size, 1)).to(device))

    # 2) Cross entropy Loss between the predicted labels D(G(z)) and fake labels 0 

    D_fake_loss = torch.nn.BCELoss()(Discriminator(Generator(z)),torch.zeros((batch_size, 1)).to(device))
...
def gan_loss_generator(Discriminator, Generator, z):
    ...
    G_loss = -torch.nn.BCELoss()(Discriminator(Generator(z)),torch.zeros((batch_size, 1)).to(device))
...
def non_saturating_gan_loss_generator(Discriminator, Generator, z):
    ...
    G_loss = torch.nn.BCELoss()(Discriminator(Generator(z)),torch.ones((batch_size, 1)).to(device))
